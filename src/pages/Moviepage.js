		...
		<div>
			...
			<div className="movietrailer">
				...
				{movie.Title !== undefined && (
					<ShowMovieTrailer title={movie.Title} apiKey={apiKey} />
				)}
				<br />
				<br />
			</div>
...
function ShowReviews(reviewArray, ratingArray, imdbID, username, setMovie) {
	...
	if (reviewtext !== undefined) {
		ownReview.review = reviewtext.review;
	}
	let idrating = ratingArray.find((x) => x.user === username);
	if (idrating !== undefined) {
		ownReview.ratingid = idrating.id;
	}
	let ratingText = ratingArray.find((x) => x.user === username);
	if (ratingText !== undefined) {
		ownReview.rating = ratingText.rating;
	}
	...
	const getRatingID = (username) => {
		return ratingArray.find((x) => x.user === username).id;
	};

	const deleteReviewRating = (ratingID, reviewID) => {
		alert("Are you sure you would like to delete this review and rating?");
		console.log(ratingID, reviewID);
		facade.deleteRating(imdbID, 0, username, ratingID);
		facade.deleteReview(imdbID, "", username, reviewID);
	};
